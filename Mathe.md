...
> Änderungen gegenüber der Erstanalyse sind mit **[AKTUALISIERT]** gekennzeichnet.
...
| **s_r(r)** (r∈R, s_r: R→Z) | Startintervall-Index der Tour r. **[AKTUALISIERT]** Formale Umrechnung: s_r(r) = 4 · start_h(r) + 1, wobei start_h(r) die Startzeit in Dezimalstunden ist (z. B. 06:45 → 6,75 → s_r = 28). |
| **e_r(r)** (r∈R, e_r: R→Z) | Endintervall-Index der Tour r. **[AKTUALISIERT]** Formale Umrechnung: e_r(r) = 4 · end_h(r) + 1, wobei end_h(r) die Endzeit in Dezimalstunden ist (z. B. 17:00 → 17,0 → e_r = 69). |
...
| **avgDv_d[t]** (t∈TD) | Durchschnittlicher Dieselverbrauch in L/100km. **[AKTUALISIERT]** avgDv_d['ActrosL'] = 0,26 (Einheit: L/100km bestätigt). |
...
| **max_p_e[t]** (t∈TE) | **[AKTUALISIERT]** Maximale Ladeleistung des E-LKW-Typs in kW. max_p_e['eActros600'] = 400; max_p_e['eActros400'] = 400. Nur für Elektro-Typen definiert. |
...
**[AKTUALISIERT]** Parameter **M** = 10.000 wurde entfernt (laut Anwender nicht mehr benötigt).
...
| **y_l[l]** (l∈L, ∈ ℤ≥0) | **[AKTUALISIERT]** Anzahl installierter Ladesäulen des Typs l. Ganzzahlig, beschränkt durch Nmax (siehe C33). |
...
| **real_p[k,l,z]** (k∈K mit type_k[k]∈TE, l∈L, z∈Z, ≥ 0) | **[AKTUALISIERT]** Reale Ladeleistung von E-LKW k an Ladesäule l zum Zeitpunkt z (kW). Nur für Elektro-LKWs definiert. |
| **P_truck[z]** (z∈Z, ≥ 0) | **[AKTUALISIERT]** Gesamte Ladeleistung aller LKWs im Zeitintervall z (kW). Formalisiert als Hilfsvariable (siehe C34). |
...
**(C11)** soc[k,1] = soc[k,96]   **[AKTUALISIERT]** ∀k∈K mit type_k[k]∈TE
...
**(C15)** Σ_{k∈K: type_k[k]∈TE} real_p[k,l,z] ≤ y_l[l] · max_p_l[l]   ∀l∈L, z∈Z   **[AKTUALISIERT]**
...
**(C17)** real_p[k,l,z] ≤ assign[k,l,z] · max_p_e[type_k[k]]   ∀k∈K mit type_k[k]∈TE, l∈L, z∈Z   **[AKTUALISIERT]**
...
**(C30)** plug[k,l,z] − plug[k,l,z+1] ≤ unplug_ok[z]   **[AKTUALISIERT]** ∀k∈K, l∈L, z=1,…,95
— Stecker darf nur gezogen werden, wenn unplug_ok = 1 (nicht zwischen 18:00 und 5:45 Uhr).

**(C31)** plug[k,l,z] ≤ 1 − depart[k,z+1]   **[AKTUALISIERT]** ∀k∈K, l∈L, z=1,…,95
...
**(C22)** p_grid[z] = Σ_{k∈K: type_k[k]∈TE} Σ_{l∈L} real_p[k,l,z] + p_l_s[z] − p_e_s[z]   ∀z∈Z   **[AKTUALISIERT]**
...
**(C28)** soc_s[z+1] = soc_s[z] + p_l_s[z] · delta_t − (1/nrt) · p_e_s[z] · delta_t   **[AKTUALISIERT]** z=1,…,95
...
**[AKTUALISIERT]** Cdiesel,var war in der Originaldatei nicht in der Gesamtkostensumme enthalten. Laut Anwender fehlt dieser Term.
